...
public class FeedProvider extends ContentProvider {
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            case ROUTE_ENTRIES:
                return NewsContract.NewsItem.CONTENT_TYPE;
            case ROUTE_ENTRIES_ID:
                return NewsContract.NewsItem.CONTENT_ITEM_TYPE;
            default:
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (uriMatch) {
            case ROUTE_ENTRIES_ID:
                ...
                builder.where(NewsContract.NewsItem._ID + "=?", id);
            case ROUTE_ENTRIES:
                // Return all known entries.
                builder.table(NewsContract.NewsItem.TABLE_NAME)
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case ROUTE_ENTRIES:
                long id = db.insertOrThrow(NewsContract.NewsItem.TABLE_NAME, null, values);
                result = Uri.parse(NewsContract.NewsItem.CONTENT_URI + "/" + id);
                break;
            case ROUTE_ENTRIES_ID:
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case ROUTE_ENTRIES:
                count = builder.table(NewsContract.NewsItem.TABLE_NAME)
                        .where(selection, selectionArgs)
                        .delete(db);
                break;
            case ROUTE_ENTRIES_ID:
                String id = uri.getLastPathSegment();
                count = builder.table(NewsContract.NewsItem.TABLE_NAME)
                       .where(NewsContract.NewsItem._ID + "=?", id)
                       .where(selection, selectionArgs)
                       .delete(db);
                break;
            default:
